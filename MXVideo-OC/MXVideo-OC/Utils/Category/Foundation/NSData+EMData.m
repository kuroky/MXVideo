...
@implementation NSData (EMData)

#pragma mark- 有关哈希函数的相关方法
...
- (NSData *)em_sha512Data {
    unsigned char result[CC_SHA512_DIGEST_LENGTH];
    CC_SHA512(self.bytes, (CC_LONG)self.length, result);
    return [NSData dataWithBytes:result length:CC_SHA512_DIGEST_LENGTH];
}

- (NSString *)em_hmacStringUsingAlg:(CCHmacAlgorithm)alg withKey:(NSString *)key {
    size_t size;
    switch (alg) {
...
- (NSData *)em_hmacDataUsingAlg:(CCHmacAlgorithm)alg withKey:(NSData *)key {
    size_t size;
    switch (alg) {
        case kCCHmacAlgMD5: size = CC_MD5_DIGEST_LENGTH; break;
        case kCCHmacAlgSHA1: size = CC_SHA1_DIGEST_LENGTH; break;
...
- (NSString *)em_hmacMD5StringWithKey:(NSString *)key {
    return [self em_hmacStringUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSData *)em_hmacMD5DataWithKey:(NSData *)key {
    return [self em_hmacDataUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSString *)em_hmacSHA1StringWithKey:(NSString *)key {
    return [self em_hmacStringUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSData *)em_hmacSHA1DataWithKey:(NSData *)key {
    return [self em_hmacDataUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSString *)em_hmacSHA512StringWithKey:(NSString *)key {
    return [self em_hmacStringUsingAlg:kCCHmacAlgSHA512 withKey:key];
}

- (NSData *)em_hmacSHA512DataWithKey:(NSData *)key {
    return [self em_hmacDataUsingAlg:kCCHmacAlgSHA512 withKey:key];
}
...
+ (NSData *)em_dataWithHexString:(NSString *)hexStr {
...
+ (NSData *)em_dataWithBase64EncodedString:(NSString *)base64EncodedString {
...
#pragma mark- 取出相应文件的NSData

+ (NSData *)em_dataNamed:(NSString *)name {
