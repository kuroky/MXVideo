...
#ifndef EM_SWAP
#define EM_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif
...
@implementation UIImage (EMAdd)

#pragma mark - 根据颜色生成图片
+ (UIImage *)em_imageWithColor:(UIColor *)color {
    return [self em_imageWithColor:color size:CGSizeMake(1, 1)];
}

#pragma mark - 根据颜色生成指定大小图片
+ (UIImage *)em_imageWithColor:(UIColor *)color size:(CGSize)size {
...
#pragma mark - 根据size创建新的图片
- (UIImage *)em_imageByResizeToSize:(CGSize)size
                        contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, [UIScreen mainScreen].scale);
    [self em_drawInRect:CGRectMake(0, 0, size.width, size.height)
...
#pragma mark - 扣出图片中指定的rect
- (UIImage *)em_imageByCropToRect:(CGRect)rect {
...
#pragma mark - Private
- (void)em_drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
...
- (UIImage *)em_imageByRotate:(CGFloat)radians fitSize:(BOOL)fitSize {
...
- (nullable UIImage *)em_imageByBlurDark {
    return [self imageByBlurRadius:40
                         tintColor:[UIColor colorWithWhite:0.11 alpha:0.73]
                          tintMode:kCGBlendModeNormal saturation:1.8 maskImage:nil];
}
...
#pragma mark - PDF图片生成
+ (UIImage *)em_imageWithPDF:(id)dataOrPath {
    return [self _yy_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)em_imageWithPDF:(id)dataOrPath size:(CGSize)size {
...
#pragma mark - Gif 生成
+ (UIImage *)em_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
...
+ (BOOL)em_isAnimatedGIFData:(NSData *)data {
...
+ (BOOL)em_isAnimatedGIFFile:(NSString *)path {
