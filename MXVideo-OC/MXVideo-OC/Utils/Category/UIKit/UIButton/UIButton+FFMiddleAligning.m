...
#import "UIButton+FFMiddleAligning.h"
...
- (UIImage *)ff_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
@implementation UIButton (FFMiddleAligning)

- (void)ff_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    CGFloat maxImageHeight = CGRectGetHeight(self.bounds) - titleSize.height - spacing * 2;
    CGFloat maxImageWidth = CGRectGetWidth(self.bounds);
    UIImage *newImage = nil;
    if (self.translatesAutoresizingMaskIntoConstraints) {
        if (imageSize.width > ceilf(maxImageWidth)) {
            CGFloat ratio = maxImageWidth / imageSize.width;
            newImage = [self.imageView.image ff_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
            imageSize = newImage.size;
        }
        if (imageSize.height > ceilf(maxImageHeight)) {
            CGFloat ratio = maxImageHeight / imageSize.height;
            newImage = [self.imageView.image ff_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
            imageSize = newImage.size;
        }
    }
